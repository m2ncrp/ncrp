...
function newmute(...) {
    ...
    msga(format("[Player '%s' has been muted by the admin %s on '%d' minutes. Reason: %s]", getPlayerName(targetid),getAccountName(playerid), time, reason), CL_RED)
...
function newban(...) {
    ...
    msga(format("[Player '%s' has been banned by the admin %s on '%d' minutes.Reason: %s]", getPlayerName(targetid),getAccountName(playerid), time, reason), CL_RED);
