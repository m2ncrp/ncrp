...
addEventHandler("onServerGuiRequested", function(guiid, name, data) {
    // store pointer to gui, to remove it later
    switch (name) {
        case "rentcar": createdGUI[guiid] <- showRentCarGUI(data.title, data.label, data.button1, data.button2); break;
        // case "taxi": createdGUI[guiid] <- showRentCarGUI(data.title, data.label, data.button1, data.button2); break; // todo other guis
    }

    // force showing true
    createdGUI[guiid]["showing"] <- true;
});

addEventHandler("onServerGuiClosed", function(guiid) {
    if (!(guiid in createdGUI)) {
        return;
    }

    local window = createdGUI[guiid].window;

    // hide gui
    if (guiIsVisible(window)) {
        guiSetVisible(window, false);
        createdGUI[guiid]["showing"] <- false;
        // showCursor(false); // todo fix for mutiple windows
    }
});

addEventHandler("onGuiElementClick", function(element) {
    foreach (guiid, object in createdGUI) {
...
function showRentCarGUI (windowText,labelText, button1Text, button2Text) {
    ...
    // if our gui is not yet created - create all elements
    if (!(guiid in createdGUI)) {
        window  = guiCreateElement( ELEMENT_TYPE_WINDOW, "", screen[0]/2 - 135, screen[1]/2 - 45, 270.0, 90.0 );
        label   = guiCreateElement( ELEMENT_TYPE_LABEL, "", 20.0, 20.0, 300.0, 40.0, false, window);
        buttons[0] = guiCreateElement(  2, "", 20.0, 60.0, 115.0, 20.0,false, window);
        buttons[1] = guiCreateElement(  2, "", 140.0, 60.0, 115.0, 20.0,false, window);
        // createdGUIetVisible( window, false);
    } else {
        // or just get created window
        window = createdGUI[guiid];
    }
    ...
    // showCursor(true); // todo fix for multiple windows

    // return window, to hide it later
    return { window = window, button1 = button[0], button2 = button[1] };
}
