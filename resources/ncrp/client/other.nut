...
addEventHandler("onAlert", function (message) {
	if(window){
		...
		label = guiCreateElement( ELEMENT_TYPE_LABEL, message, 10.0, 20.0, 300.0, 20.0, false, window);
		button = guiCreateElement( ELEMENT_TYPE_BUTTON, "ОК" ,  10.0, 70, 50.0, 20.0, false, window);
	}
	if(!oldCursorState){
		showCursor(true);
	}
