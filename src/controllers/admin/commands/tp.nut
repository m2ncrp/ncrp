 ...
 *     /pp 0 15 32.0
 */
acmd("pp", function(playerid, playerSessiodId1 = null, playerSessiodId2 = null, offset = 2.0) {
    local targetid1 = getPlayerIdByPlayerSessionId(playerSessiodId1.tointeger());
    local targetid2 = getPlayerIdByPlayerSessionId(playerSessiodId2.tointeger());
    ...
    if (isPlayerInVehicle(targetid1)) {
        return msg(playerid, "Player is in the vehicle", CL_WARNING);
    }

    local pos = getPlayerPosition(targetid2);
    setPlayerPosition(targetid1, pos[0], pos[1], pos[2] + offset.tofloat());
    msg(playerid, format("You've successuly tp'd player #%s to player %s", playerSessiodId1, playerSessiodId2), CL_SUCCESS);
});
 ...
 *     /pv 0 15 32.0
 */
acmd("pv", function(playerid, playerSessiodId = null, targetid2 = null, offset = 1.2) {
    local targetid1 = getPlayerIdByPlayerSessionId(playerSessiodId.tointeger());
    local targetid2 = toInteger(targetid2);
    ...
    local pos = getVehiclePosition(targetid2);
    setPlayerPosition(targetid1, pos[0], pos[1], pos[2] + offset.tofloat());
    msg(playerid, format("You've successuly tp'd player #%s to vehicle %d", playerSessiodId, targetid2), CL_SUCCESS);
});
 ...
 *     /vp 0 15 32.0
 */
acmd("vp", function(playerid, targetid1 = null, playerSessiodId = null, offset = 0.1) {
    local targetid1 = targetid1.tointeger();
    local targetid2 = getPlayerIdByPlayerSessionId(playerSessiodId.tointeger());
    ...
    local pos = getPlayerPosition(targetid2);
    setVehiclePosition(targetid1, pos[0] + 2.0, pos[1], pos[2] + offset.tofloat());
    msg(playerid, format("You've successuly tp'd vehicle #%d to player %s", targetid1, playerSessiodId), CL_SUCCESS);
});
