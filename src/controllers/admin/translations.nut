alternativeTranslate({
    "en|inappropriatebehavior" : "inappropriate behavior"
    "ru|inappropriatebehavior" : "неуместное поведение"

    "en|admin.error"                        :   "Error: id empty or player not connected."
    "ru|admin.error"                        :   "Ошибка: id не указан или игрок не подключен."

    "en|admin.kick.hasbeenkicked"           :   "[SERVER] You has been kicked for: %s."
    "ru|admin.kick.hasbeenkicked"           :   "[CЕРВЕР] Вы были кикнуты по причине: %s."

    "en|admin.kick.kicked"                  :   "You've kicked %s for: %s."
    "ru|admin.kick.kicked"                  :   "Вы кикнули %s по причине: %s."



    "en|admin.mute.youhave"                 :   "[SERVER] You are muted."
    "ru|admin.mute.youhave"                 :   "[СЕРВЕР] Вы заглушены."

    "en|admin.mute.muted"                   :   "Player %s has been muted on %d minutes. Reason: %s"
    "ru|admin.mute.muted"                   :   "Игрок %s заглушен на %d минут. Причина: %s"

    "en|admin.mute.unmuted"                 :   "Player %s has been unmuted."
    "ru|admin.mute.unmuted"                 :   "Заглушка чата с игрока %s снята."

    "en|admin.mute.notmuted"                :   "Player %s is not muted."
    "ru|admin.mute.notmuted"                :   "Игрок %s не заглушен."



    "en|admin.ban.banned"                   :   "Player with id %s has been banned until %d"
    "ru|admin.ban.banned"                   :   "Игрок c id %s забанен до %s"

    "en|admin.ban.unbanned"                 :   "Player %s has been unbanned"
    "ru|admin.ban.unbanned"                 :   "Игрок %s разбанен"

    "en|admin.ban.reason"                   :   "Reason: %s."
    "ru|admin.ban.reason"                   :   "Причина: %s."

    "en|admin.ban.speedhack-vehicle"        :   "Speedhack (trainer or modification game files)"
    "ru|admin.ban.speedhack-vehicle"        :   "Спидхак (трейнер или модификация файлов игры)"

    "en|admin.ban.trainer-onfoot"           :   "Using trainer (speed-hack)"
    "ru|admin.ban.trainer-onfoot"           :   "Использование трейнера (спидхак)"

    "en|admin.ban.trainer-weapons"          :   "Unlawful receipt of weapons"
    "ru|admin.ban.trainer-weapons"          :   "Неправомерное получение оружия"

    "en|admin.ban.trainer-fuel"             :   "Using trainer (endless fuel)"
    "ru|admin.ban.trainer-fuel"             :   "Использование трейнера (бесконечное топливо)"
/*
    "en|admin.warn.msga.playerreceived"     :   "Player %s received a warning."
    "ru|admin.warn.msga.playerreceived"     :   "Игрок %s получил одно предупреждение."

    "en|admin.warn.msga.removefromplayer"   :   "Warning removed from player %s."
    "ru|admin.warn.msga.removefromplayer"   :   "С игрока %s снято одно предупреждение"
*/
})
