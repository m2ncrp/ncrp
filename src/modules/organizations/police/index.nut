...
POLICE_RANK_SALLARY_PERMISSION_SKIN <- [ // calculated as: (-i^2 + 27*i + 28)/200; i - rank number
   /* 0.14 */ [0.14, policeRankPermission(true, false, false),  [172, 173, 174] ], // 0 - "police.cadet"
   /* 0.27 */ [0.18, policeRankPermission(true, true, false),   [172, 173, 174] ], // 1 - "police.patrol"
   /* 0.39 */ [0.22, policeRankPermission(true, true, false),   [172, 173, 174] ], // 2 - "police.officer"
   /* 0.50 */ [0.26, policeRankPermission(true, true, true),    [69]     ], // 3 - "police.detective"
   /* 0.60 */ [0.30, policeRankPermission(true, true, true),    [172, 173, 174] ], // 4 - "police.sergeant.wa1"
   /* 0.69 */ [0.34, policeRankPermission(true, true, true),    [172, 173, 174] ], // 5 - "police.sergeant.2"
   /* 0.77 */ [0.38, policeRankPermission(true, true, true),    [172, 173, 174] ], // 6 - "police.lieutenant.1"
   /* 0.84 */ [0.42, policeRankPermission(true, true, true),    [172, 173, 174] ], // 7 - "police.lieutenant.2"
   /* 0.90 */ [0.46, policeRankPermission(true, true, true),    [172, 173, 174] ], // 8 - "police.Captain.1"
   /* 0.95 */ [0.50, policeRankPermission(true, true, true),    [172, 173, 174] ], // 9 - "police.Captain.2"
   /* 0.99 */ [0.54, policeRankPermission(true, true, true),    [172, 173, 174] ], // 10 - "police.Captain.3"
   /* 1.02 */ [0.58, policeRankPermission(true, true, true),    [172, 173, 174] ], // 11 - "police.commander"
   /* 1.04 */ [0.62, policeRankPermission(true, true, true),    [172, 173, 174] ], // 12 - "police.deputychief"
   /* 1.05 */ [0.66, policeRankPermission(true, true, true),    [172, 173, 174] ], // 13 - "police.assistantchief"
   /* 1.05 */ [0.70, policeRankPermission(true, true, true),    [175] ]  // 14 - "police.chief"
];
    ...
    if (isPlayerInPoliceVehicle(playerid) && seat == 0) {
        ...
        if ( isOfficer(playerid)) {
            ...
            if (!isOnPoliceDuty(playerid)) {
                ...
                privateKey(playerid, "b", "policeCarRuporBinder", policeCarRuporBinder);
                privateKey(playerid, "v", "policeCarRadioBinder", policeCarRadioBinder);
                policeCarRuporBinderCreator(playerid);
                policeCarRadioBinderCreator(playerid);
            }
        }
    }
...
event("onPlayerVehicleExit", function( playerid, vehicleid, seat ) {
    ...
    if (isVehicleidPoliceVehicle(vehicleid)) {
        if (isOfficer(playerid) && isOnPoliceDuty(playerid) ) {
            ...
            removePrivateKey(playerid, "b", "policeCarRuporBinder");
            removePrivateKey(playerid, "v", "policeCarRadioBinder");
            policeCarRuporBinderRemover(playerid);
            policeCarRadioBinderRemover(playerid);
        }
    }
});
